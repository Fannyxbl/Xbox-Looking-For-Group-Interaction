...
class User_Info:
    ...
    async def retrieve_user_info(self, session: aiohttp.ClientSession, token):
        try:
            ...
            async with session.get('https://profile.xboxlive.com/users/me/profile/settings?settings=Gamertag', headers=headers) as user_info_request:
